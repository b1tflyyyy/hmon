    ...
    void MonitorInfoView::Init()
    {
        ...
        root_ = ftxui::window(ftxui::text(" Information "),
            ftxui::vbox({
              ...
              field_row("Refresh rate", std::format("{:.2f}", monitor_.refresh_rate_)),
              field_row("X position", std::to_string(monitor_.x_pos_)),
              field_row("Y position", std::to_string(monitor_.y_pos_)),
              field_row("Scale", std::format("{:.2f}", monitor_.scale_)),
